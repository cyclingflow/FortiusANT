    ...
    def GetTrainer(clv, AntDevice=None):
        ...
        if dev != False:
            ...
            #command line argument used
                ...
                return clsSimplifiedTacx1901UsbTrainer(clv, msg, hu, dev)
            else:
...
class clsTacx1901UsbTrainer(clsTacxUsbTrainer):
    ...
    def __init__(self, clv, Message, Headunit, UsbDevice):
        ...
        if (None == self.powerFactors4Level):
            self.powerFactors4Level, self.grade4Level, self.speedAtKnot = self.defaultPowerFactors4Level, self.defaultGrade4Level, self.defaultSpeedAtKnot
        else:
            self.customFactors = True
            #self.speedAtKnot = self._findMinimumKnotPosition()
            self.speedAtKnot = self.defaultSpeedAtKnot
            #print(selfself.speedAtKnot)
    ...
    def _parseParameters(self, filename, line, userGradeMode):
    ...
    def _parseAntifierPowerCurveFactors(self, filename):
        ...
        for line in factorFile:
            ...
            #parameter?
            if ('=' in line):
                userGradeMode = self._parseParameters(filename, line, userGradeMode)
...
#           print('__AvoidCycleOfDeath', self.SpeedKmh, Resistance)
    ...
    def _ReceiveFromTrainer(self):
        ...
        if len(data) < 40:
            ...
            #print(tuple[nAccell], tuple[nAvePower])
...
class clsSimplifiedTacx1901UsbTrainer(clsTacx1901UsbTrainer):
    ...
    def __init__(self, clv, Message, Headunit, UsbDevice):
        ...
        if (os.path.exists(powerGradeFactors1901_filename)):
            userGradeMode, customGrades, customSlopeFactor, customInterceptFactor = \
                 self._parseCustomPowerGrade(powerGradeFactors1901_filename)
            if customGrades != None:
                self.grade4Level = customGrades
            if customSlopeFactor != None:
                self.slopeFactor = customSlopeFactor
            if customInterceptFactor != None:
                self.interceptFactor = customInterceptFactor
    ...
    def _parseLine(self, filename, line, userGradeMode, customGrades, customSlopeFactor, customInterceptFactor):
        parameter, value = self._parseParameterValue(line)
        if debug.on(debug.Function):logfile.Write ("{}._parseAParameters():" \
                                                   "found parameter {} = value {} in factor file {}.".format(self._name, parameter, value, filename))
        ...
        #userGrades = [-4.3, -3.4, -2.5, -1.6, -0.7, 0.5, 1.4, 2.9, 4.0, 5.6, 7.4, 8.6, 9.8, 11.0]
            ...
            try:
                ...
                if userGradeMode == None: userGradeMode = True
            except:
        ...
        #Slope factor = [a, b]
        elif (parameter == 'customslopefactor'):
            exception = False
            try:
                customSlopeFactor = eval(value)
            except:
                exception = True
                customSlopeFactor = None
            if (exception or len(customSlopeFactor)!= 2 or not isinstance(customSlopeFactor[0], float)):
                if debug.on(debug.Function):logfile.Write ("{}._parseAntifierPowerCurveFactors():" \
                                                           "error parsing power factor file {}: Unknown value {}.".format(self._name, filename, value))
        ...
        #Intercept factor = [a, b]
            ...
            exception = False
            try:
                customInterceptFactor = eval(value)
            except:
                exception = True
        ...
        return userGradeMode, customGrades, customSlopeFactor, customInterceptFactor



    def _parseCustomPowerGrade(self, filename):
        userGradeMode = None
        customGrades =  None
        customSlopeFactor = None
        customInterceptFactor = None
        ...
        for line in factorFile:
            ...
            if ('=' in line):
                userGradeMode, customGrades, customSlopeFactor, customInterceptFactor = \
                    self._parseLine(filename, line, userGradeMode, customGrades,
                                    customSlopeFactor, customInterceptFactor)
        if userGradeMode==None and customGrades==None:
            userGradeMode = False
        return userGradeMode, customGrades, customSlopeFactor, customInterceptFactor
